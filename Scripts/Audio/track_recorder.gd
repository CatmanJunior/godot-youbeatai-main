...
func _on_recording_progress_updated(track_index: int, percentage: float) -> void:
	recording_sample_button.update_button(percentage)
	# Update waveform progress for synth tracks
	if SongState.current_section:
		var track: TrackData = SongState.current_section.tracks[track_index]
		if track.track_type == TrackData.TrackType.SYNTH:
			waveform_visualizer.update_progress(track_index, percentage)

func _on_synth_sequence_ready(track_index: int) -> void:
	# Update waveform visualization when synth voice processing is complete
	if not SongState.current_section:
		return
	var track: TrackData = SongState.current_section.tracks[track_index]
	if track.recording_data:
		waveform_visualizer.update_waveform(track_index, track.recording_data)
