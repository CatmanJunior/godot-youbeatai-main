...
func _nsdf_pitch(frame: PackedFloat32Array) -> Vector2:
	...
	# Running energy terms for normalization.
	var energy_start := 0.0
	var energy_end := 0.0
	for i in range(n):
		energy_start += frame[i] * frame[i]
	...
	for tau in range(min_lag, max_lag + 1):
		...
		# Energy normalization: sum of x[i]^2 for i in [0, n-tau-1]
		# plus sum of x[i+tau]^2 for i in [0, n-tau-1].
		# Incrementally compute m(tau).
		var m_left := 0.0
		var m_right := 0.0
		for i in range(limit):
			m_left += frame[i] * frame[i]
			m_right += frame[i + tau] * frame[i + tau]
