...
public partial class BeatSprite : Sprite2D
{
	...
	{
		if (!Visible || Manager.instance.settingsPanel.Visible) return;

        if (inputEvent is InputEventMouseButton mouseButtonEvent && mouseButtonEvent.ButtonIndex == MouseButton.Left && mouseButtonEvent.IsReleased())
        {
            OnClick();
        }
	}
