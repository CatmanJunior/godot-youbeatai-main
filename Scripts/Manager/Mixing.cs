...
public partial class Manager : Node
{
	...
	private void SynthMixing_UpdateMixingVolumesForSynth(int synth, float mastervolume, Vector3? givenWeights = null)
	{
		...
		if (synth == 0)
		{
			...
			AudioServer.SetBusVolumeLinear(AudioServer.GetBusIndex("GreenVoice"), weightsToUseThisTime.Y * mastervolume);
		...
		if (synth == 1)
		{
			...
			AudioServer.SetBusVolumeLinear(AudioServer.GetBusIndex("PurpleVoice"), weightsToUseThisTime.Y * mastervolume);
	...
	private void SongMixing_UpdateMixingVolumesForSong(float mastervolume)
	{
		...
		AudioServer.SetBusVolumeLinear(AudioServer.GetBusIndex("SongSynth"), weights.Z * mastervolume);
