...
public partial class Manager : Node
{
	...
	private void SynthMixing_UpdateMixingVolumesForSynth(int synth, float mastervolume)
	{
		if (synth == 0)
		{
			...
			GetNode<Node>("/root/scene/Managers/LayerVoiceOver0/bass_line").Set("volume_linear",  weights.Z * mastervolume);

		}
		if (synth == 1)
		{
			layerVoiceOver1.audioPlayer.VolumeLinear = weights.Y * mastervolume * 1.5f;
			GetNode<Node>("/root/scene/Managers/LayerVoiceOver1/synth_line").Set("volume_linear", weights.Z * mastervolume);
		}
	}
