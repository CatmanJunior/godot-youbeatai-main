...
public partial class Manager : Node
{
    ...
    private void InitButtonActions()
    {
        ...
        foreach (var button in emojiButtons) button.ButtonUp += () => { AddLayer(currentLayerIndex + 1, button.Icon); CloseEmojiPrompt(); addedLayer = true; };
    ...
    private void UpdateBeatSprites(double delta)
    {
        ...
        for (int beat = 0; beat < BpmManager.beatsAmount; beat++)
        {
            for (int ring = 0; ring < 4; ring++)
            {
                ...
                if (active && showTemplate) sprite.Modulate = new Color(255, 255, 255, 0.5f);
            }
        }
    }
