...
func get_map() -> Dictionary:
	...
	return {
		...
		O.START_SHORT_TIMER:       _outcome_start_short_timer,
		O.BEGIN_STOMP_PHASE:       _outcome_stomp_setup,
		O.END_STOMP_PHASE:         _outcome_stomp_done,
		O.SHOW_CLAP_RING:          _outcome_show_clap_ring,
		...
		O.SHOW_MIX_STAR:           _outcome_show_mix_star,
		O.ON_MIX_STAR_REACHED:     _outcome_mix_star_reached,
		O.SHOW_OUTSIDE_STAR:       _outcome_show_outside_star,
		O.ON_OUTSIDE_STAR_REACHED: _outcome_outside_star_reached,
...
func _outcome_start_short_timer() -> void:
...
func _outcome_show_clap_ring() -> void:
	EventBus.track_sprites_visibility_requested.emit(Tutorial.INDEX_ORANGE_RING, true)
...
func _outcome_show_mix_star() -> void:
	_show_star_area(tutorial.in_between_area, tutorial.in_between_mesh)

func _outcome_mix_star_reached() -> void:
	_reach_star_area(tutorial.in_between_area, tutorial.in_between_mesh)

# ── Outside star ──────────────────────────────────────────────────────────────

func _outcome_show_outside_star() -> void:
	_show_star_area(tutorial.out_side_area, tutorial.out_side_mesh)

func _outcome_outside_star_reached() -> void:
	_reach_star_area(tutorial.out_side_area, tutorial.out_side_mesh)
