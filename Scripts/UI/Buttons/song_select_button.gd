...
func update_ui():
	set_button_selected(GameState.song_mode)
	
func set_button_selected(_active: bool) -> void:
	super.set_button_selected(GameState.song_mode)

	song_track_line_texture.visible = GameState.song_mode
	if song_section_progress_bar != null:
		song_section_progress_bar.visible = GameState.song_mode

func _on_press():
	print("pressed")
	GameState.song_mode = !GameState.song_mode
	set_button_selected(GameState.song_mode)
	super._on_press()
