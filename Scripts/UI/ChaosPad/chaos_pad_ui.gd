...
# Tutorial zone targets — hidden by default, shown by TutorialOutcomes
@export var first_target: TextureRect
@export var mix_target: TextureRect
@export var outside_target: TextureRect
...
func set_knob_position(pos: Vector2, retry_count := 0) -> void:
	if pos == TrackData.KNOB_POSITION_UNSET:
		...
		if tri.size() >= 3:
			...
			# Defer to next frame, but bound the number of retries to avoid infinite recursion
			# if tri never becomes populated (e.g. due to scene setup issues).
			if retry_count < 3:
				set_knob_position.call_deferred(pos, retry_count + 1)
			else:
				push_warning("set_knob_position: knob.container.tri not populated after 3 deferred retries; giving up.")
		return
